...
abstract class REST_Controller extends \CI_Controller {
    ...
    public function response($data = NULL, $http_code = NULL)
    {
        ...
        elseif ($data !== NULL)
        {
            // $data['metadata']['end_microtime']  = microtime(TRUE);
            // if (isset($data['metadata']['start_microtime']))
            // {
            //     $data['metadata']['execution_time'] = $data['metadata']['end_microtime'] - $data['metadata']['start_microtime'];
            // }
    ...
    {
        $my_number = $this->getCurrentNumber($seq);

        if($my_number != null && count($my_number) > 0)
        {
            $this->updateNextNumber($seq);
            return $my_number[0]->GEN_NUMBER;
        }

        $this->insertNextNumber($seq, $text);
        return $this->getNextNumber($seq, $text);
    }
    ...
    private function insertNextNumber($seq, $text)
    {
        ...
        try 
        {
            ...
            $CI->db->insert(LEGACY.".GEN_LOOKUP_NUMB", $preparedData);
    ...
    protected function _log_request($authorized = FALSE)
    {
        ...
        $id = $this->getNextNumber(AN_SOS_12_SEQ, AN_SOS_12_DESC);
        
        if ($this->_args)
        {
            if (isset($this->_args['EVIDENCES']))
            {
                unset($this->_args['EVIDENCES']);
            }

            if (isset($this->_args['token']))
            {
                unset($this->_args['token']);
            }

            if (isset($this->_args['EVIDENCE']))
            {
                unset($this->_args['EVIDENCE']);
            }
        }
